...
%mice in each group
eyfpmice = [747 759 762 764 771 780 782];
chr2mice = [744 746 758 761 763 781 783];

%Day labels
daykey = {'PR Training' ; 'PR Test'};     
%these mice didn't undergo extinction and instead did the progressive ratio
%in S4.4C
    ...
    if sum(mouse_ID == eyfpmice) == 1 || sum(mouse_ID == chr2mice) == 1 
        ...
        %Cycle through all days in a given mouse for each of the variables 
            for row = 1:size(raw_mouse,1)
                ...
                %cycle through variables: active, inactive, rewards,
                %receptacle, final PR value, stims,
                %note that Current PR value is in same place as timeout
                %previously
                %and grab the data
                for variable = [9 10 11 12 17 18]
...
allvariablename = { 'active', 'inactive', 'rewards', 'receptacle', 'final_PR_value', 'stims',};
