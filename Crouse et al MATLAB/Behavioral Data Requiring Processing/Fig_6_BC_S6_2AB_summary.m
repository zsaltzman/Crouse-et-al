...
%exp = Figs
exp = 'Fig 6 B-C + S6.2A-B';

%mice in each group
eyfpmice = [682 693 701 710 721 724];
chr2mice = [685 702 711 712 722];
yokemice = [678 695 709 720 723];
...
groupdata = {'eyfpdata'; 'chr2data' ; 'yokedata' };
groupdataindex = {'eyfpdataindex','chr2dataindex', 'yokedataindex'};
...
for num = 1:length(all_mouse_ID)
    ...
    if sum(mouse_ID == eyfpmice) == 1 || sum(mouse_ID == chr2mice) == 1 || sum(mouse_ID == yokemice) == 1
        ...
        elseif sum(mouse_ID == yokemice) == 1
            group = 3;
            yokedataindex = yokedataindex + 1;
            yokemouseorder(yokedataindex) = mouse_ID;
        end
        ...
        for row = 1:size(raw_mouse,1)
            ...
            variableindex = 1;
                ...
                %eyfp and chr2 mice stims were counted individually after PT due to a different code setup,
                %instead of sets of 40 (2 sec of 20 Hz) so divide by 40 to
                %match yoke
                if variable == 18 && group ~= 3 && row >4
    ...
    raw_mouse = cat(1,raw(1,:), raw_mouse);
...
%go through all variables and cat them as: eyfp,chr2,yoke with an extra col of
...
allmouseorder = cat(2,eyfpmouseorder,NaN, chr2mouseorder,NaN,yokemouseorder);
emptydaycell = {NaN};
...
for allvariable = 1:size(allvariablename,2)
    ...
    %cat all group data
    eval([num2str(allvariablename{allvariable}),'= cat(2, eyfpdata{' num2str(allvariable), '}, emptycol, chr2data{' num2str(allvariable), '},emptycol, yokedata{', num2str(allvariable), '});']);
    %allvariablename{allvariable} = cat(2, eyfpdata{allvariable}, chr2data{allvariable}, yokedata{allvariable})
