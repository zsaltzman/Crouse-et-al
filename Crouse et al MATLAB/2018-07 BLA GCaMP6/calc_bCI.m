...
event_dir_path = FP_INDIVIDUAL_DAY_GRAPH_DATA;
...
for current_var=1:length(variable_list)
    output_dir_path = [ output_parent_dir '\' variable_names(variable_list(current_var)) ];
    event_variable = variable_list(current_var);
    
    MDIR_DIRECTORY_PATH = output_dir_path;
    make_directory; 
    
    for fidx=1:length(event_dir)
        fname = event_dir{fidx};
        % unwrap fname if necessary
        if isa(fname, 'cell')
            fname = fname{1};
        end
