...
daykey = {'PT 1' ; 'PT 2'; 'PT 3'; 'PT 4' ; 'Training 1'; 'Training 2';... 
    ...
    'Training 8';'Training 9'; 'Training 10'; 'Training 11'; 'Training 12'; ...
        'Training 13';'Training 14'; 'Ext 1'; 'Ext 2'};     


%% Import the data
raw = readcell([folder '\2018-07 ' exp '.xlsx']);

%for this exp, mice were ID'd starting with letters, so convert just to the
%mouse's number for simplicity here
for row = 2:size(raw,1)
    rename_mice(row-1,1) = str2double(raw{row,1}(4));
end

raw(2:end,1) = num2cell(rename_mice);
...
for num = 1:length(all_mouse_ID)
    ...
    if sum(mouse_ID == mice) == 1 
        ...
        if mouse_ID == 4
           raw_mouse = [raw_mouse(1:17,:) ; nan_rows ; raw_mouse(18:end,:)];
        elseif mouse_ID == 6
            raw_mouse = [raw_mouse(1:12,:) ; nan_rows ; raw_mouse(13:end,:)];
            raw_mouse = [raw_mouse(1:17,:) ; nan_rows ; raw_mouse(18:end,:)];
        end
...
for allvariable = 1:size(allvariablename,2)
    ...
    %allvariablename{allvariable} =  eyfpdata{allvariable}
