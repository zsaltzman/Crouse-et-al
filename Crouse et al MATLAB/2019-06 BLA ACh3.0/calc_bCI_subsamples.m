...
% Downsample incorrect responses to have the same size as correct responses
% Takes random indices from incorrect each time, so results may not be
% entirely consistent
downsample_means = cell(length(filenames), 1);
for graphidx=1:size(graphdata, 1)
    incorr_size = size(graphdata{graphidx, 3}, 2);
    corr_size = size(graphdata{graphidx, 1}, 2);
    
    if incorr_size > corr_size
        downsamples_idx = sort(randperm(incorr_size, corr_size));
        incorr_data = graphdata{graphidx, 3};
        downsample_means{graphidx} = mean(incorr_data(:, downsamples_idx), 2);
    else
        downsample_means{graphidx} = graphmean{graphidx, 3};
    end
end

graphdata(:, 3) = downsample_means;
