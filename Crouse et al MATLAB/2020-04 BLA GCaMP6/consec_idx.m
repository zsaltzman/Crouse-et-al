function [c_logical, c_idx] = consec_idx(indices,threshold)
...
k = [true,diff(indices) > threshold];
s = cumsum(k);
x =  histc(s,1:s(end));
idx = find(k);
consecutive = idx(x>=threshold);

c_idx = cell(size(consecutive, 2), 1);
c_logical = false(size(indices));
for c=1:length(consecutive)
   x_idx = consecutive(c) == idx;
   c_idx{c} = consecutive(c):(consecutive(c)+x(x_idx)-1);
   c_logical(consecutive(c):(consecutive(c)+x(x_idx)-1)) = true;
end  
